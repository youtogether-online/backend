...
func (cq *ChatQuery) QueryRoom() *RoomQuery {
...
func (cq *ChatQuery) QueryUser() *UserQuery {
...
func (cq *ChatQuery) WithRoom(opts ...func(*RoomQuery)) *ChatQuery {
	query := (&RoomClient{config: cq.config}).Query()
...
func (cq *ChatQuery) WithUser(opts ...func(*UserQuery)) *ChatQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withRoom; query != nil {
		if err := cq.loadRoom(ctx, query, nodes,
			func(n *Chat) { n.Edges.Room = []*Room{} },
			func(n *Chat, e *Room) { n.Edges.Room = append(n.Edges.Room, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withUser; query != nil {
		if err := cq.loadUser(ctx, query, nodes,
			func(n *Chat) { n.Edges.User = []*User{} },
			func(n *Chat, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadRoom(ctx context.Context, query *RoomQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Room)) error {
	...
	query.Where(predicate.Room(func(s *sql.Selector) {
		s.Where(sql.InValues(chat.RoomColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.room_chat
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_chat" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "room_chat" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *ChatQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Chat)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(chat.UserColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.chat_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "chat_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
